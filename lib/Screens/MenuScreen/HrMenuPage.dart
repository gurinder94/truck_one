...
class _HrMenuPageState extends State<HrMenuPage> {
  ...
  gridMenuList(BuildContext context) {
    return GridView.count(
      ...
      shrinkWrap: true,
      children: List.generate(hrMenuItems.length, (index) {
        return menuItem(hrMenuItems[index], context);
      }),
    );
  }
  ...
  openMenuItems(String title, BuildContext context) {
    switch (title) {
      ...
      //       context, MaterialPageRoute(builder: (context) =>
      //       ChangeNotifierProvider(
      //           create: (_) => WishListProvider(), child: WishListScreen())));
      //   break;
    }
  }
